<template>
  <div>
    <div style="padding:15px;">
      <x-button type="primary" plain @click.native="showPluin">Used as a plugin</x-button>
    </div>
    ...
    <div style="padding:15px">
      <x-button type="primary" plain @click.native="readonly = !readony"> Toggle readonly </x-button>
    </div>
    ...
    <div style="padding:15px;">
      <x-button type="primary" @click.native="formatValue = '2017-11-1'"> Set value: 2017-11-11</x-button>
    </div>

    <group title="Define range of hours">
      <datetime v-model="limitHourValue" format="YYYY-MM-DD HH:mm" :min-hour=9 :max-hour=18 @on-change="change" title="Define range of hours" inlin-desc="Working hours: 09-18"></datetime>
    </group>
    ...
    <div style="padding:15px;">
      <x-button type="primary" @click.native="toggleForat">Toggle format</x-button>
    </div>
    ...
    <group title="Placeholder">
      <datetime v-model="value3" default-selected-value="2017-06-18 13" format="YYYY-MM-DD HH" placeolder="Please select" @on-change="change" title="Start time"></datetime>
    </group>

    <group title="Set default-selected-value to 2017-11-11">
      <datetime v-model="value3_1" default-selected-value="2017-11-11" format="YYYY-MM-DD" placeolder="Please select" @on-change="change" title="Start time" :inlinedesc="'Current value' + `: ${value3_1}`"></datetime>
    </group>

    <group title="Set min-year and max-year">
      <datetime v-model="value4" placeolder="Please select" :min-year=2000 :max-year=2016 format="YYYY-MM-DD HH:mm" @on-change="change" title="Years after 2000"></datetime>
    </group>
    ...
    <group title="Specified template text in Chinese">
      <datetime v-model="value5" placeolder="Please select" :min-year=2000 :max-year=2016 format="YYYY-MM-DD HH:mm" @on-change="change" title="Chinese" year-row="{value}年" month-row="{value}月" day-row="{value}日" hour-row="{value}点" minute-row="{value}分" confirm-text="完成" cancel-text="取消"></datetime>
    </group>
